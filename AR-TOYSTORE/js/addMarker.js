AFRAME.registerComponent("create-markers",{
    init:async function(){
        ...
        toys.map(toy => {
            ...
            ratingplane.setAttribute("material" , {coloe: "#F0C30F"})
            ratingplane.setAttribute("visible" , false)

            var ratings = document.createElement("a-entity")
            ratings.setAttribute("id", `ratings-of-${toy.id}`)
            ratings.setAttribute("position", { x: 0 , y: 0.05 , z: 0.1})
            ratings.setAttribute("rotation" , { x: 0 , y: 0 , z: 0})
            ratings.setAttribute("text",{font: "monoid", width: 2.4, color: "brown" , value: `Customer Rating: \n${toy.last_rating}`})
            ratingplane.appendChild(ratings)
            ...
            reviewplane.setAttribute("geometry", { primitive: "plane" , width: 1.5 , height: 0.5 })
            reviewplane.setAttribute("material" , {coloe: "#F0C30F"})
            reviewplane.setAttribute("visible" , false)

            var reviews = document.createElement("a-entity")
            reviews.setAttribute("id", `review-of-${toy.id}`)
            reviews.setAttribute("position", { x: 0 , y: 0.05 , z: 0.01})
            reviews.setAttribute("rotation" , { x: 0 , y: 0 , z: 0})
            reviews.setAttribute("text",{font: "monoid", width: 2.4, color: "brown" , value: `Customer Review: \n${toy.last_review}`})
            reviewplane.appendChild(reviews)
